...
const UserMyBookings = () => {
  ...
  const bookings = [
    ...
    },
    {
      ...
      status: "completed",
      avatar:
        "https://res.cloudinary.com/dipdvqnin/image/upload/v1774722839/completed_livr0l.jpg",
    },
  ];
    ...
    <div className="pb-20 pt-[85px] px-4 sm:px-6 bg-white min-h-screen">
      <div className="max-w-[848px] mx-auto space-y-6">
        ...
        <div className="bg-[linear-gradient(90deg,_#EFF6FF_0%,_#FAF5FF_100%)] p-5 rounded-xl flex justify-between items-center border border-[#E0E7FF]">
          ...
          <button
            type="button"
            className="bg-black text-white text-xs px-5 py-2.5 rounded-lg font-semibold shadow-sm"
          >
            Find Artisans
          </button>
        </div>
          ...
          {filtered.length > 0 ? (
            filtered.map((item) => (
              <UserMyBookingsCard
                key={item.id}
                item={item}
                onViewContract={() => navigate("/client-contract")}
              />
            ))
          ) : (
